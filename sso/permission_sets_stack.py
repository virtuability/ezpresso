        ...
        for permission_set in permission_sets:
            ...
            if "inline_policy_document_file" in permission_set:
                inline_policy_document = (
                    file_sub(f"{permission_set['inline_policy_document_file']}")
                )
            else:
                inline_policy_document = None

            sso_permission_set = sso.CfnPermissionSet(self, f"{permission_set['permission_set_name']}",
                ...
                for ou_assignment in permission_set['ou_assignments']:
                    ...
                    for group_assignment in permission_set['group_assignments']:
                        ...
                        for account in account_structure.get_accounts():
                            ...
                            if account['name_path'] == ou_path or (ou_recursive and account['name_path'].startswith(ou_path + '/')):
                                accounts_groups_mappings.append({'account_id': account['Id'], 'group_name': group_assignment['name']})
            ...
            if permission_set.get("account_assignments") is not None:
                for account_assignment in permission_set['account_assignments']:
                    for group_assignment in permission_set['group_assignments']:
                        ...
                        accounts_groups_mappings.append({'account_id': account_assignment, 'group_name': group_assignment['name']})
            ...
            for accounts_groups_mapping in dedup:
                ...
                sso_assignment = sso.CfnAssignment(self, f"{permission_set['permission_set_name']}-{account_id}-{group_name_prefix}{group_name}",
                    instance_arn=properties['sso_instance_arn'],
